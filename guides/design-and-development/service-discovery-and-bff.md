...
This document describes the current service discovery approach and the BFF plan.

## Phase 1: BFF proxy and static config

- The sveltekit web app exposes `/api/*` endpoints as a thin proxy to microservices.
- The top-most level, +layout.svelte fetches config from `web/src/routes/api/config/+server.ts` during server-load and provides it to all pages.
